...
/*
  MAX = 8   // 棋盘大小
  int queen[MAX]    // 一维结果数组，每个皇后的坐标为(i, queen[i]), i = 1, 2, ... MAX
...
char*** solveNQueens(int n, int* returnSize) {
    int *queen = (int *) malloc (n * sizeof(int));   /* 每个皇后的坐标为 (n, queen[n]) */
...
void nqueen (int row, int *queen) {
    ...
    for (i = 0; i < N; ++i) {
        *(queen + row) = i;   /* 将横坐标为row的皇后，从第一列到第N列，全部测试一遍 */
...
bool check (int row, int *queen) {
    ...
    for (i = 0; i < row; ++i) {      /* 判断当前n皇后与之前的皇后是否  同列，同对角线 */
        if (queen[row] == queen[i] || abs(queen[row] - queen[i]) == (row - i))
            return false;   /* 若满足不合法条件，则返回不合法位置 */
    }
    return true;
}
